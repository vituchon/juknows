...
import (
	...
	"github.com/vituchon/juknows/presentation/util"
	"github.com/vituchon/juknows/presentation/web/controllers"
...
const (
	storeKeyFilePath = ".ss_juknows" // the file were the actual key is stored
)
...
func StartServer() {
	...
	log.Printf("Juknows web server listening at port %v", server.Addr)
	err = server.ListenAndServe()
	if err != nil {
		log.Println("Unexpected error initiliazing juknows web server: ", err)
	}
...
func buildRouter() *mux.Router {
	...
	apiGet("/sessions", controllers.GetSessions)
	apiGet("/session", controllers.GetClientSession)
	apiPut("/sessions/{id:[0-9]+}", controllers.UpdateSession)
	apiDelete("/sessions/{id:[0-9]+}", controllers.DeleteSession)

	apiPost("/request-life-proff", controllers.RequestLifeProff)
	apiPost("/provide-life-proff", controllers.ProvideLifeProff)
	apiGet("/last-provided-life-proff", controllers.GetLastProvidedLifeProff)
